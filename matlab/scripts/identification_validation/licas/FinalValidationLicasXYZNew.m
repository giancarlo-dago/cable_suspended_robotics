...
p = [ 5.0000    1.3109    0.8242    0.3629    4.2369    1.3299    0.1739    0.3126    0.5170    0.7449   17.8552         0    0.0211    0.5559];
...
for i=1:nTraj
    ...
    qModel{i}(1,4:6) = [0 -qModel{i}(1,2) -qModel{i}(1,3)];
    qModel{i}(1,7:14) = q0{i};
    qdModel{i}(1,1:6) = zeros(1,6);
    qdModel{i}(1,7:14) = qd0{i};
    shoulderPoseModel{i}(1,:) = shoulderPoseReal0{i};

    for k=2:(length(tModel{i}))
        [qModel{i}(k,:), qdModel{i}(k,:)] = qFunModelLicasNew(p,qModel{i}(k-1,:),qdModel{i}(k-1,:),Ts_sim,qRefVecInterp{i}(k-1,:),qdRefVecInterp{i}(k-1,:));
    end
    shoulderPoseModel{i} = [L*sin(qModel{i}(:,3)) L*sin(qModel{i}(:,2)) qModel{i}(:,1)+qModel{i}(:,4)];                                                           % Compute the position of the shoulders again
...
e = abs(shoulderPoseRealVecInterp{i}(:,j) - shoulderPoseModel{i}(:,j));
plot(tModel{i},e, 'k--', 'Linewidth', 1.5 ,'Color', [0.2, 0.2, 0.2])
legend('$r_m^x$','$r_s^x$','$|e^x|$','Orientation','horizontal'); xlabel('t [s]'); ylabel('x [m]'); set(gca, 'FontSize',16); set(gca, 'XLim', [tModel{i}(j) tModel{i}(end)]);
set(gca, 'YLim', [-0.3 0.3], 'YTick', [-0.3:0.1:0.3]);
grid on; box on; set(gcf,'color','w');
R = corrcoef(shoulderPoseRealVecInterp{i}(:,j),shoulderPoseModel{i}(:,j));
rho = R(1,2);
str = {['$|\overline{e}|=',num2str(round(max(e),4)),'\,m$'],['$\rho=',num2str(round(rho,3)),'$']};
annotation('textbox', [0.303, 0.265, 0.2, 0.1], 'String',str,'interpreter','latex', 'BackgroundColor', 'w', 'FitBoxToText','on','FontSize',14,'VerticalAlignment','middle')
...
annotation('textbox', [0.752, 0.265, 0.2, 0.1], 'String',str,'interpreter','latex', 'BackgroundColor', 'w', 'FitBoxToText','on','FontSize',14,'VerticalAlignment','middle')

% exportgraphics(h, 'LicasX2Id.pdf');
% exportgraphics(h, 'LicasXId.pdf'); 

%% IDENTIFICATION: Y EXPERIMENTS
j=2;  i=3;
h = figure('Renderer', 'painters', 'Position', [10 10 900 300]);
...
e = abs(shoulderPoseRealVecInterp{i}(:,j) - shoulderPoseModel{i}(:,j));
plot(tModel{i},e, 'k--', 'Linewidth', 1.5 ,'Color', [0.2, 0.2, 0.2])
legend('$r_m^y$','$r_s^y$','$|e^y|$','Orientation','horizontal'); xlabel('t [s]'); ylabel('[m]'); set(gca, 'FontSize',16); set(gca, 'XLim', [tModel{i}(j) tModel{i}(end)]);
set(gca, 'YLim', [-0.3 0.3], 'YTick', [-0.3:0.1:0.3]);
grid on; box on; set(gcf,'color','w');
R = corrcoef(shoulderPoseRealVecInterp{i}(:,j),shoulderPoseModel{i}(:,j));
rho = R(1,2);
str = {['$|\overline{e}|=',num2str(round(max(e),3)),'\,m$'],['$\rho=',num2str(round(rho,3)),'$']};
annotation('textbox', [0.31, 0.265, 0.1, 0.1], 'String',str,'interpreter','latex', 'BackgroundColor', 'w', 'FitBoxToText','on','FontSize',14,'VerticalAlignment','middle')

% exportgraphics(h, 'LicasY1Id.pdf');

% IDENTIFICATION: Y EXPERIMENT
j=2;  i=4;
% h = figure('Renderer', 'painters', 'Position', [10 10 900 300]);
...
rho = R(1,2);
str = {['$|\overline{e}|=',num2str(round(max(e),3)),'\,m$'],['$\rho=',num2str(round(rho,3)),'$']};
annotation('textbox', [0.752, 0.265, 0.1, 0.1], 'String',str,'interpreter','latex', 'BackgroundColor', 'w', 'FitBoxToText','on','FontSize',14,'VerticalAlignment','middle')

% exportgraphics(h, 'LicasY2Id.pdf');
exportgraphics(h, 'LicasYId.pdf');

%% IDENTIFICATION: Z EXPERIMENTS
j=3;  i=5;
h = figure('Renderer', 'painters', 'Position', [10 10 900 300]);
...
e = 180/pi*abs(shoulderPoseRealVecInterp{i}(:,j) - shoulderPoseModel{i}(:,j));
plot(tModel{i},e, 'k--', 'Linewidth', 1.5 ,'Color', [0.2, 0.2, 0.2])
legend('$\phi_m^z$','$\phi_s^z$','$|e^z|$','Orientation','horizontal'); xlabel('t [s]'); ylabel('$\phi$ [deg]'); set(gca, 'FontSize',16); set(gca, 'XLim', [tModel{i}(j) tModel{i}(end)]);
set(gca, 'YLim', [-60 60], 'YTick', [-60:20:60]);
grid on; box on; set(gcf,'color','w');
R = corrcoef(180/pi*shoulderPoseRealVecInterp{i}(:,j),180/pi*shoulderPoseModel{i}(:,j));
rho = R(1,2);
str = {['$|\overline{e}|=',num2str(round(max(e),3)),'\, ^o$'],['$\rho=',num2str(round(rho,3)),'$']};
annotation('textbox', [0.31, 0.265, 0.1, 0.1], 'String',str,'interpreter','latex', 'BackgroundColor', 'w', 'FitBoxToText','on','FontSize',14,'VerticalAlignment','middle')

% exportgraphics(h, 'LicasZ1Id.pdf');

% IDENTIFICATION: Z EXPERIMENT
j=3;  i=6;
% h = figure('Renderer', 'painters', 'Position', [10 10 900 300]);
...
rho = R(1,2);
str = {['$|\overline{e}|=',num2str(round(max(e),3)),'\, ^o$'],['$\rho=',num2str(round(rho,3)),'$']};
annotation('textbox', [0.75, 0.265, 0.1, 0.1], 'String',str,'interpreter','latex', 'BackgroundColor', 'w', 'FitBoxToText','on','FontSize',14,'VerticalAlignment','middle')

% exportgraphics(h, 'LicasZ2Id.pdf');
exportgraphics(h, 'LicasZId.pdf');
...
e = abs(shoulderPoseRealVecInterp{i}(:,j) - shoulderPoseModel{i}(:,j));
plot(tModel{i},e, 'k--', 'Linewidth', 1.5 ,'Color', [0.2, 0.2, 0.2])
legend('$r_m^x$','$r_s^x$','$|e^x|$','Orientation','horizontal'); xlabel('t [s]'); ylabel('[m]'); set(gca, 'FontSize',16); set(gca, 'XLim', [tModel{i}(j) tModel{i}(end)]);
set(gca, 'YLim', [-0.3 0.3], 'YTick', [-0.3:0.1:0.3]);
grid on; box on; set(gcf,'color','w');
R = corrcoef(shoulderPoseRealVecInterp{i}(:,j),shoulderPoseModel{i}(:,j));
rho = R(1,2);
str = {['$|\overline{e}|=',num2str(round(max(e),3)),'\,m$'],['$\rho=',num2str(round(rho,3)),'$']};
annotation('textbox', [0.32, 0.275, 0.1, 0.1], 'String',str,'interpreter','latex', 'BackgroundColor', 'w', 'FitBoxToText','on','FontSize',14,'VerticalAlignment','middle')
...
rho = R(1,2);
str = {['$|\overline{e}|=',num2str(round(max(e),3)),'\,m$'],['$\rho=',num2str(round(rho,3)),'$']};
annotation('textbox', [0.8, 0.275, 0.1, 0.1], 'String',str,'interpreter','latex', 'BackgroundColor', 'w', 'FitBoxToText','on','FontSize',14,'VerticalAlignment','middle')

exportgraphics(h, 'Licas_X,Y_Val.pdf');
...
e = 180/pi*abs(shoulderPoseRealVecInterp{i}(:,j) - shoulderPoseModel{i}(:,j));
plot(tModel{i},e, 'k--', 'Linewidth', 1.5 ,'Color', [0.2, 0.2, 0.2])
legend('$\phi_m^z$','$\phi_s^z$','$|e^z|$','Orientation','horizontal'); xlabel('t [s]'); ylabel('[deg]'); set(gca, 'FontSize',16); set(gca, 'XLim', [tModel{i}(j) tModel{i}(end)]);
set(gca, 'YLim', [-60 60], 'YTick', [-60:20:60]);
grid on; box on; set(gcf,'color','w');
R = corrcoef(180/pi*shoulderPoseRealVecInterp{i}(:,j),180/pi*shoulderPoseModel{i}(:,j));
rho = R(1,2);
str = {['$|\overline{e}|=',num2str(round(max(e),3)),'\, ^o$'],['$\rho=',num2str(round(rho,3)),'$']};
annotation('textbox', [0.33, 0.275, 0.1, 0.1], 'String',str,'interpreter','latex', 'BackgroundColor', 'w', 'FitBoxToText','on','FontSize',14,'VerticalAlignment','middle')
...
legend('$r_m^x$','$r_s^x$','$|e^x|$','Orientation','horizontal');  ylabel('[m]'); set(gca, 'FontSize',16); set(gca, 'XLim', [tModel{i}(j) tModel{i}(end)]);
set(gca, 'YLim', [-0.2 0.4], 'XTick',[]);
grid on; box on; set(gcf,'color','w');
R = corrcoef(shoulderPoseRealVecInterp{i}(:,j),shoulderPoseModel{i}(:,j));
rho = R(1,2)
...
exportgraphics(h, 'Licas_Z,XY_Val.pdf');
