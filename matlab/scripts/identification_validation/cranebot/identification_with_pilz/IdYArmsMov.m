...
TRAJ = {'ModArmsMovExpY1_amp0.2', 'ModArmsMovExpY1_amp0.3', 'ModArmsMovExpY1_amp0.35','ModNatResExpY3', 'ModNatResExpY1', 'ModNatResExpY2'};
nTraj = length(TRAJ);
figure
for i=1:nTraj
...
%     nSamples(i) = length(realTraj);
    ...
    timeRealVec{i} = time;
    platformPoseRealVec{i} = [zeros(length(time),1) platformY zeros(length(time),1)];
...
massCablesPlatform = 206;
%  
% errorfun = @(p) [x1 - modelPlanarArms('Y',p(4),massCablesPlatform,p(1),p(5),p(2),p(3),p(6),p(7),p(8),Ts_sim,platformPose0_1,qRefMat{1},qdRefMat{1},t1), ...
%                  x2 - modelPlanarArms('Y',p(4),massCablesPlatform,p(1),p(5),p(2),p(3),p(6),p(7),p(8),Ts_sim,platformPose0_2,qRefMat{2},qdRefMat{2},t2), ...
%                  x3 - modelPlanarArms('Y',p(4),massCablesPlatform,p(1),p(5),p(2),p(3),p(6),p(7),p(8),Ts_sim,platformPose0_3,qRefMat{3},qdRefMat{3},t3), ...
%                  x4 - modelPlanarArms('Y',p(4),massCablesPlatform,p(1),p(5),p(2),p(3),p(6),p(7),p(8),Ts_sim,platformPose0_4,qRefMat{4},qdRefMat{4},t4), ...
%                  x5 - modelPlanarArms('Y',p(4),massCablesPlatform,p(1),p(5),p(2),p(3),p(6),p(7),p(8),Ts_sim,platformPose0_5,qRefMat{5},qdRefMat{5},t5), ...
%                  x6 - modelPlanarArms('Y',p(4),massCablesPlatform,p(1),p(5),p(2),p(3),p(6),p(7),p(8),Ts_sim,platformPose0_6,qRefMat{6},qdRefMat{6},t6)];
% 
errorfun = @(p) [x1 - modelPlanarArms('Y',p,Ts_sim,platformPose0_1,qRefMat{1},qdRefMat{1},t1), ...
                 x2 - modelPlanarArms('Y',p,Ts_sim,platformPose0_2,qRefMat{2},qdRefMat{2},t2), ...
                 x3 - modelPlanarArms('Y',p,Ts_sim,platformPose0_3,qRefMat{3},qdRefMat{3},t3)];

% % errorfun = @(p) [x1 - modelPlanarArms(p(4),massCablesPlatform,p(1),p(5),p(2),p(3),p(6),p(7),p(8),Ts_sim,platformPose0_1,qRefMat{1},qdRefMat{1},t1)];
% 
% Fit the model to the first signal, constraining the coefficients
eps = [3.7 0 250 0.5 5 5000 0.05 0.05 100];
p0 = [3.7 206 250 0.5 20 16000 0.05 0.05 100];
lb = p0 - eps;
ub = p0 + eps;
...
% fvC kd kp lCz fsC iA1xx iA2xx iCxx
% f = [300  13  2500    3.7    0.5    0    0   0];
% f = [250  20  16000    3.7    0.5    0.0    0.0   100];

% lCz,mC,fvC,fsC,kd,kp,iA1xx,iA2xx,iCxx
% f = [3.7 206 250 0.5 20 16000 0.0 0.0 100];
% f = [3.64 206 250 0.5 25 25000 0.05 0.05 100];
% f = [3.7 206 250 0.5 20 16000 0.05 0.05 100];
% f = [3.7 206 250 0.5 20 16000 0 0 100];

% lCz,mC,fvC,fsC,kd,kp,iA1xx,iA2xx,iCxx
f = [3.64 206 485.3 0.767 20 16000 0.05 0.05 102.3];

%    1.0e+04 *
% 
%   Columns 1 through 6
% 
%    0.000365921903237   0.020600000000000   0.048537764757808   0.000076707986696   0.002098271588580   1.100069283354467
% 
%   Columns 7 through 9
% 
%    0.000000003600519   0.000000483118823   0.000049215764020


x1_sim = modelPlanarArms('Y',f,Ts_sim,platformPose0_1,qRefMat{1},qdRefMat{1},t1);
x2_sim = modelPlanarArms('Y',f,Ts_sim,platformPose0_2,qRefMat{2},qdRefMat{2},t2);
x3_sim = modelPlanarArms('Y',f,Ts_sim,platformPose0_3,qRefMat{3},qdRefMat{3},t3);
x4_sim = modelPlanarArms('Y',f,Ts_sim,platformPose0_4,qRefMat{4},qdRefMat{4},t4);
x5_sim = modelPlanarArms('Y',f,Ts_sim,platformPose0_5,qRefMat{5},qdRefMat{5},t5);
x6_sim = modelPlanarArms('Y',f,Ts_sim,platformPose0_6,qRefMat{6},qdRefMat{6},t6);
...
% 
% tModelZ = load("tModelZ.mat");
% qRefMatZ = load("qRefMatZ.mat");
% 
% subplot(1,2,1)
...
% legend('$q_{xy,1}^d$','$q_{xy,2}^d$','$q_{xy,3}^d$','Orientation','vertical','FontSize',14); xlabel('t [s]'); ylabel('[deg]'); set(gca, 'FontSize',18); set(gca, 'XLim', [tModel{1}(1) 30]);
% set(gca, 'YLim', [-22.5 22.5], 'YTick',[-22.5:7.5:22.5]);
% 
% subplot(1,2,2)
% plot(tModelZ.tModel{1},180/pi*qRefMatZ.qRefMat{1}(:,1), 'k-', 'Linewidth', lw ,'Color', [0.6, 0.6, 0.6]), legend, grid on, hold on
% plot(tModelZ.tModel{2},180/pi*qRefMatZ.qRefMat{2}(:,1), 'k-', 'Linewidth', lw ,'Color', [0.4, 0.4, 0.4]), legend, grid on, hold on
% plot(tModelZ.tModel{3},180/pi*qRefMatZ.qRefMat{3}(:,1), 'k-', 'Linewidth', lw ,'Color', [0.2, 0.2, 0.2]), legend, grid on, hold on
% legend('$q_{z,1}^d$','$q_{z,2}^d$','$q_{z,3}^d$','Orientation','vertical','FontSize',14); xlabel('t [s]'); ylabel('[deg]'); set(gca, 'FontSize',18); set(gca, 'XLim', [tModelZ.tModel{1}(1) 30]);
% set(gca, 'YLim', [-10 10], 'YTick',[-12:4:12]);
% 
%% Plot for paper
...
legend('${r}_m^y$','${r}_s^y$','$|e^y|$','Orientation','horizontal'); xlabel('t [s]'); ylabel('y [m]'); set(gca, 'FontSize',18); set(gca, 'XLim', [t3(1) 40]);
set(gca, 'YLim', [-0.04 0.04], 'YTick',[-0.04:0.02:0.04]);
...
exportgraphics(h, 'CranebotArmsMovY_amp0.35.pdf');
...
h2 = figure('Renderer', 'painters', 'Position', [10 10 900 300]);
A = readmatrix('proposedModelCranebot.txt');
B = readmatrix('doublePendModelCranebot.txt');
C = readmatrix('singPendModelCranebot.txt');

shoulderPose = x3_sim(13080:13080+29999)
time = 0:0.001:30-0.001;
% plot(time, 1.10.*shoulderPose), hold on
% plot(time,[A(13080:13080+29999,2)])
% plot(time,[C(1:30000,2)])
% plot(time,[B(1:30000,2)-B(1,2)-0.0267])

hold on
plot(time,abs(A(13080:13080+29999,2)-movmean(B(1:30000,2)-B(1,2)-0.0267,100)), 'k-', 'Linewidth', lw ,'Color', [0.6, 0.6, 0.6])
plot(time,abs(A(13080:13080+29999,2)-movmean(C(1:30000,2),100)), 'k-', 'Linewidth', lw ,'Color', [0.4, 0.4, 0.4])
plot(time,abs(A(13080:13080+29999,2)-1.10.*shoulderPose), 'k-', 'Linewidth', lw ,'Color', [0.2, 0.2, 0.2])

legend('$(i)$ Double pend.','$(ii)$ Single pend.','$(iii)$ Proposed','Orientation','horizontal'); 
xlabel('t [s]'); ylabel('$|e_y|$ [m]'); set(gca, 'FontSize',16); set(gca, 'XLim', [0 25]);
set(gca, 'YLim', [-inf 0.04], 'YTick', [0:0.01:0.04]);
grid on; box on; set(gcf,'color','w');

%% Computation max error
doublePendMaxError = max(abs(A(13080:13080+29999,2)-movmean(B(1:30000,2)-B(1,2)-0.0267,100)))
singlePendMaxError = max(abs(A(13080:13080+29999,2)-movmean(C(1:30000,2),100)))
proposedMaxError = max(abs(A(13080:13080+29999,2)-1.10.*shoulderPose))
