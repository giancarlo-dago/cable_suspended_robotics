...
[spA,spdA,spddA] = trapveltraj([0 distA], CampioniA(1));
spA(:,end) = spA(:,end-1);
spdA(:,end) = spdA(:,end-1);
spddA(:,end) = spddA(:,end-1);
...
%         [spA(end+1:end+CampioniA(j-1)), spdA(end+1:end+CampioniA(j-1)), spddA(end+1:end+CampioniA(j-1))] = lspb(0, distA, CampioniA(j-1));
        [spA(end+1:end+CampioniA(j-1)), spdA(end+1:end+CampioniA(j-1)), spddA(end+1:end+CampioniA(j-1))] = trapveltraj([0 distA], CampioniA(j-1));
        spA(:,end) = spA(:,end-1);
        spdA(:,end) = spdA(:,end-1);
        spddA(:,end) = spddA(:,end-1);
    else                                               % circonferenza
...
[spB,spdB,spddB] = trapveltraj([0 distB], CampioniB(1));
spB(:,end) = spB(:,end-1);
spdB(:,end) = spdB(:,end-1);
spddB(:,end) = spddB(:,end-1);
...
%         [spB(end+1:end+CampioniB(j-1)), spdB(end+1:end+CampioniB(j-1)), spddB(end+1:end+CampioniB(j-1))] = lspb(0, distB, CampioniB(j-1));
        [spB(end+1:end+CampioniB(j-1)), spdB(end+1:end+CampioniB(j-1)), spddB(end+1:end+CampioniB(j-1))] = trapveltraj([0 distB], CampioniB(j-1));
        spB(:,end) = spB(:,end-1);
        spdB(:,end) = spdB(:,end-1);
        spddB(:,end) = spddB(:,end-1);
    else                                            % circonferenza
...
[sopA,sopdA,sopddA] = trapveltraj([0 distA], CampioniA(1));
sopA(:,end) = sopA(:,end-1);
sopdA(:,end) = sopdA(:,end-1);
sopddA(:,end) = sopddA(:,end-1);
...
%     [sopA(end+1:end+CampioniA(j-1)), sopdA(end+1:end+CampioniA(j-1)), sopddA(end+1:end+CampioniA(j-1))] = lspb(0, distA, CampioniA(j-1));
    [sopA(end+1:end+CampioniA(j-1)), sopdA(end+1:end+CampioniA(j-1)), sopddA(end+1:end+CampioniA(j-1))] = trapveltraj([0 distA], CampioniA(j-1));
    sopA(:,end) = sopA(:,end-1);
    sopdA(:,end) = sopdA(:,end-1);
    sopddA(:,end) = sopddA(:,end-1);
end
...
[sopB,sopdB,sopddB] = trapveltraj([0 distB], CampioniB(1));
sopB(:,end) = sopB(:,end-1);
sopdB(:,end) = sopdB(:,end-1);
sopddB(:,end) = sopddB(:,end-1);
for j = 3:length(OB)
...
%     [sopB(end+1:end+CampioniB(j-1)), sopdB(end+1:end+CampioniB(j-1)), sopddB(end+1:end+CampioniB(j-1))] = lspb(0, distB, CampioniB(j-1));
    [sopB(end+1:end+CampioniB(j-1)), sopdB(end+1:end+CampioniB(j-1)), sopddB(end+1:end+CampioniB(j-1))] = trapveltraj([0 distB], CampioniB(j-1));
    sopB(:,end) = sopB(:,end-1);
    sopdB(:,end) = sopdB(:,end-1);
    sopddB(:,end) = sopddB(:,end-1);
end
...
subplot(2,1,2); plot(time,p(2,:),'b'); xlabel('[s]'); ylabel('[m]');legend('$y$','Interpreter','Latex'); grid; xlim([time(1) time(end)]);
figure(4);
...
subplot(2,1,2); plot(time,p(4,:),'b'); xlabel('[s]'); ylabel('[m]');legend('$y$','Interpreter','Latex'); grid; xlim([time(1) time(end)]);
figure(7);
...
figure(9)
plot(time,oB); grid; xlabel('[s]'); ylabel('[rad]'); xlim([time(1) time(end)]);
