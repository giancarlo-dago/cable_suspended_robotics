...
% th2A_0 = 0;     % posizione iniziale giunto 2 manipolatore A
% th3A_0 = 0;     % posizione iniziale giunto 3 manipolatore A
% th1B_0 = 0;         % posizione iniziale giunto 1 manipolatore B
% th2B_0 = 0;      % posizione iniziale giunto 2 manipolatore B
% th3B_0 = 0;      % posizione iniziale giunto 3 manipolatore B
...
% Simulate end effectors tracking 
run('ee_trajectories_new'); 
close all;
sim('second_arm_compensation3')
q_controlled = ans.q_controlled;

% % Simulate uncontrolled case
...
% 
% figure();
% plot(q_controlled.Time, q_controlled.Data(:,1)); hold on;
% plot(q_uncontrolled.Time, q_uncontrolled.Data(:,1)); 
% legend('contr','uncontr','Location','southeast'); title('alfa comparison'); grid; xlabel('[s]'); ylabel('[deg]'); ylim([-0.05 0.05])
% 
% figure();
% plot(q_controlled.Time, q_controlled.Data(:,2)); hold on;
% plot(q_uncontrolled.Time, q_uncontrolled.Data(:,2));
% legend('contr','uncontr','Location','southeast'); title('beta comparison'); grid; xlabel('[s]'); ylabel('[deg]'); ylim([-1 4])
