function tau = parloop_recursive_invdyn_tree_sym_f(theta, dtheta, ddtheta, g, info, msg)
...
%     Vd = info.Vd;
    ...
    prev_joint_index = info.previous_joint_index;
    prev_frame_index = info.previous_frame_index;
    ...
    parfor i=1:n_frames
        if (frame_type(i) == link_frame_type)
            current_S = sym(S(:,prev_joint_index(i)));
...
%             prev_index = info.previous_frame_index(i);
            M_bi_prev = sym(M_bi(:,:,prev_frame_index(i)));
            result = sym(Mbi\M_bi_prev);
        end 
        M_mutual(:,:,i) = result;
        if (frame_type(i) == link_frame_type)
            j = i;
%             prev_joint_index = previous_joint_index(i);
            th = theta(prev_joint_index(i));
        	T_mutual(:,:,i) = sym(expm(-bracket_f(A(:,j))*th) * M_mutual(:,:,i));      % Computation T_{i,i-1}: tranformation matrix in a general configuration 
        else
    ...
    parfor i=1:n_frames
        if (frame_type(i) == link_frame_type)
            j = i;
            G(:,:,i) = sym(G_f(mass(j),Inertia(:,:,j),inertial_disp(j,:)));                  % Referring the spatial inertia matrix in the i-th frame
        end
    end
    ...
    for i=1:n_frames
        if (frame_type(i) == link_frame_type)
            ...
            dth = dtheta(prev_joint_index(i));
            ddth = ddtheta(prev_joint_index(i));
            if i == 1
                V_prev = V_0;
                Vd_prev = Vd_0;
            else
%                 V_prev = V_0;
%                 Vd_prev = Vd_0;
                V_prev = V(:,prev_frame_index(i));
                Vd_prev = Vd(:,prev_frame_index(i));
            end
