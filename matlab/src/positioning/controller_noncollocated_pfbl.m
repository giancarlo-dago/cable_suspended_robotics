...
for i=1:length(time)
    ...
    e = q_ref - q_passive;
    E(:,i) = e;
    e_dot = q_dot_ref - q_dot_passive;
    E_DOT(:,i) = e_dot;
    in = Kp*e + Kd*e_dot + q_ddot_ref;
   
    %% Noncollocated Partial FBL
    ...
    % Pseudoinverse
    q0 = [0 0 0 0 0 0 0 0]';
%     y = -pinv(Bbm)*(Bb*in+nb) + (eye(8)-Bbm'*Bbm)*(Kd_nullspace*(-q_dot(5:12)) + Kp_nullspace*(q0-q(5:12)));
%     s_min = 0;
%     k = 0;
    ...
    if (s_min >= eps)
        ...
        k_sq = (1-(s_min/eps)^2)*k_sq_max;
    end
    Bbm_star = Bbm'*inv(Bbm*Bbm'+k_sq*eye(4));
    y = -Bbm_star*(Bb*in+nb) + (eye(8)-Bbm'*Bbm)*(Kd_nullspace*(-q_dot(5:12)) + Kp_nullspace*(q0-q(5:12)));
    k = sqrt(k_sq);
